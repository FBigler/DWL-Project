...
import sqlalchemy
import os
import jupyter_notebooks.config_lake_RDS_ZIPCode as creds
...
def fetch_ZIPCode_api_json():
    url = "https://swisspost.opendatasoft.com/api/v2/catalog/datasets/plz_verzeichnis_v2/exports/csv"
    return url

def convert_into_dataframe(url):
    df=pd.read_csv(url,on_bad_lines='skip',sep=";")
    df = df[["postleitzahl","kanton"]]
    return df
...
def lambda_handler(event, context):
    ...
    df = convert_into_dataframe(fetch_ZIPCode_api_json())
