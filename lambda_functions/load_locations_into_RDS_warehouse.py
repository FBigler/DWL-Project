...
def sql_query():
        ...
        query_all = """SELECT * FROM chargingstations_locations"""
        return query_all
...
def lambda_handler(event, context):
    ...
    df_location = pd.read_sql(sql_query(),con=engine_lake)
    df_location = df_location.rename(columns={"longitude":"coordinate_east","latitude":"coordinate_nord"})

    # upload data into warehgouse table
    df_location.to_sql(name=DB_TABLE_warehouse, con=engine_warehouse, if_exists="append", index=False)
